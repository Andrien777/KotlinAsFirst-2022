...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    val intermediate = mutableMapOf<String, MutableList<Double>>()
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
    var cheapCost = Double.MAX_VALUE
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean =
    (chars.map { it.lowercase()[0] }).toSet().containsAll(word.lowercase().toSet())
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    ...
    val lettersToDelete = mutableListOf<String>()
    for (letter in list)
        if (answer[letter] != null) {
            answer[letter] = answer[letter]!! + 1
            lettersToDelete.remove(letter)
        } else {
            answer[letter] = 1
...
fun propagateHandshakes(friends: Map<String, Set<String>>): Map<String, Set<String>> {
    ...
    for (key in answer.keys)
        answer[key] = answer[key]!!.toMutableSet()
    var changed = true
    while (changed) {
        val answerCopy = mutableMapOf<String, MutableSet<String>>()
        for (key in answer.keys)
            answerCopy[key] = answer[key]!!.toMutableSet()
        for ((key, value) in answer)
            for (person in value)
                if (answerCopy[person] != null) {
                    answerCopy[key] = answerCopy[key]!!.union(answerCopy[person]!!).toMutableSet()
                    answerCopy[key]!!.remove(key)
                } else
                    answerCopy[person] = mutableSetOf()
        changed = answer != answerCopy
        for (key in answerCopy.keys)
            answer[key] = answerCopy[key]!!.toMutableSet()
    }
    return answer
}

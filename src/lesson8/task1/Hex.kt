...
data class Hexagon(val center: HexPoint, val radius: Int) {
     ...
     */
    fun distance(other: Hexagon): Int = max(0, this.center.distance(other.center) - this.radius - other.radius)
...
class HexSegment(val begin: HexPoint, val end: HexPoint) {
    ...
    fun direction(): Direction {
        ...
        return if (this.begin.y == this.end.y) {
            if (this.begin.x > this.end.x) Direction.LEFT
            else Direction.RIGHT
        } else if (this.begin.x == this.end.x) {
            if (this.begin.y > this.end.y) Direction.DOWN_LEFT
            else Direction.UP_RIGHT
        } else {
            if (this.begin.y > this.end.y) Direction.DOWN_RIGHT
...
fun neighbours(point: HexPoint): List<HexPoint> {
    val res = mutableListOf<HexPoint>()
    for (dx in -1..1)
        for (dy in -1..1)
            if (point.distance(HexPoint(point.x + dx, point.y + dy)) == 1)
                res.add(HexPoint(point.x + dx, point.y + dy))
    return res
}

fun makePath(from: HexPoint, to: HexPoint, prev: List<HexPoint>): List<HexPoint> {
    if (from == to) return prev
    val currentDistance = from.distance(to)
    for (pt in neighbours(from)) {
        if (pt.distance(to) < currentDistance)
            return makePath(pt, to, prev + pt)
    }
    return listOf()
}

fun pathBetweenHexes(from: HexPoint, to: HexPoint): List<HexPoint> = makePath(from, to, listOf(from))

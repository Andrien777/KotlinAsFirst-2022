...
fun rookMoveNumber(start: Square, end: Square): Int {
    ...
    return if (start.row == end.row || start.column == end.column) 1 else 2
}
...
fun rookTrajectory(start: Square, end: Square): List<Square> {
    ...
    if (rookMoveNumber(start, end) == 1) return ans + end
    ans.add(Square(start.column, end.row))
    return ans + end
}
...
fun bishopMoveNumber(start: Square, end: Square): Int {
    ...
    return if ((start.column + start.row == end.column + end.row) || (start.column - start.row) == (end.column - end.row)) 1 else 2
}
...
fun kingTrajectory(start: Square, end: Square): List<Square> {
    ...
    while (sq != end) {
        ans.add(sq)
        sq = Square(sq.column + sign(end.column - sq.column), sq.row + sign(end.row - sq.row))
    }
    return ans + end
}
...
fun findAdjacent(sq: Square): List<Square> {
    val ans = mutableListOf<Square>()
    for (dx in -2..2) {
        for (dy in -2..2) {
            if (abs(dx) + abs(dy) == 3) {
                val test = Square(sq.column + dx, sq.row + dy)
                if (test.inside())
                    ans.add(test)
            }
        }
...
fun knightMoveNumber(start: Square, end: Square): Int {
    if (!start.inside() || !end.inside()) throw IllegalArgumentException()
    if (start == end) return 0
    val list = findAdjacent(start).toMutableList()
    for (i in 1..6) {
        if (end in list) return i
        val bound = list.size
        for (j in 0 until bound) {
            list.addAll(findAdjacent(list[0]))
            list.removeAt(0)
        }
    }
    return -1
...
fun knightTrajectory(start: Square, end: Square): List<Square> {
    ...
    while (sq != end) {
        path.add(sq)
        val testDst = knightMoveNumber(sq, end)
        for (test in findAdjacent(sq)) {
            if (knightMoveNumber(test, end) < testDst) {
                sq = test
                break
            }
        }
